...
impl Ticket {
    ...
    fn new(title: String, description: String, status: String) -> Self {
        let title = match title.len() {
            n if n == 0 => panic!("Title cannot be empty"),
            n if n > 50 => panic!("Title cannot be longer than 50 characters"),
            _ => title,
        };
        let description = match description.len() {
            n if n == 0 => panic!("Description cannot be empty"),
            n if n > 500 => panic!("Description cannot be longer than 500 characters"),
            _ => description,
        };
